...
# Inject synchronous classes at the top-level before other modules import them
...
class TestTier2BoundaryCases(BaseNeDotifyTestCase):
    ...
    def test_f3_boundary_cache_expiry(self):
        ...
        cached = self.core.db.get_cached_stream("youtube", "yt_id")
        self.assertIsNone(cached['stream_url'])
...
class TestTier3CrossFeature(BaseNeDotifyTestCase):
    ...
    def test_f3_cross_youtube_play_cache(self):
        """Cross-Feature: Resolving YT stream caches stream, subsequent query pulls from cache."""
        ...
        cached = self.core.db.get_cached_stream("youtube", "yt_cache_id")
        self.assertIsNotNone(cached)
        self.assertEqual(cached['stream_url'], 'http://example.com/stream.mp3')

    def test_f3_cross_history_recommendations(self):
        """Cross-Feature: Play tracks to populate history -> generate recommendations using history."""
        # Add a track

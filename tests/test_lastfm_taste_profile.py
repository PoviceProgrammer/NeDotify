...
def test_lastfm_key_rotation():
    ...
    with patch("services.lastfm_service.API_KEYS", list(test_keys)):
        service = LastFMService()
        keys_seen = []
        for _ in range(len(test_keys) * 2):
            keys_seen.append(service._get_next_api_key())
        
        assert keys_seen[0] == test_keys[0]
        assert keys_seen[1] == test_keys[1]
        assert keys_seen[2] == test_keys[2]
        assert keys_seen[3] == test_keys[3]
        assert keys_seen[len(test_keys)] == test_keys[0]  # full cycle wraps around


def test_lastfm_api_queries_mocked():
    with patch("services.lastfm_service.API_KEYS", ["test_key_mock"]):
        service = LastFMService()
...
def test_lastfm_caching_and_ttl():
    with patch("services.lastfm_service.API_KEYS", ["test_key_caching"]):
        service = LastFMService()
...
def test_lastfm_offline_graceful_fallback():
    with patch("services.lastfm_service.API_KEYS", ["test_key_offline"]):
        service = LastFMService()

...
function setupZapretPanel() {
    const toggleZapret = document.getElementById('toggle-zapret');
    const badge = document.getElementById('zapret-status-badge');
    const info = document.getElementById('zapret-binary-info');
    const inputCustomArgs = document.getElementById('input-zapret-custom-args');
    const inputBinPath = document.getElementById('input-zapret-bin-path');

    function updateBadge(running, binaryFound) {
        if (badge) {
            if (running) {
                badge.textContent = '● Активен (Работает)';
                badge.style.background = 'rgba(16, 185, 129, 0.2)';
                badge.style.color = '#10b981';
            } else {
                badge.textContent = 'Не активен';
                badge.style.background = 'rgba(255, 255, 255, 0.1)';
                badge.style.color = 'var(--text-sec)';
            }
        }
        if (info) {
            info.textContent = binaryFound
                ? 'Исполняемый файл winws.exe найден'
                : 'Внимание: winws.exe не найден. Поместите winws.exe в %APPDATA%\\.nedotify\\zapret\\ или укажите путь ниже';
            info.style.color = binaryFound ? 'var(--text-sec)' : '#f59e0b';
        }
    }
    ...
    // Strategy Cards
    document.querySelectorAll('#zapret-mode-select .opt-card').forEach(card => {
        card.addEventListener('click', () => {
            document.querySelectorAll('#zapret-mode-select .opt-card').forEach(c => c.classList.remove('active'));
            card.classList.add('active');
            selectedMode = card.dataset.val;
    ...
    async function applyZapret(enable) {
        const customArgs = inputCustomArgs ? inputCustomArgs.value.trim() : '';
        const binPath = inputBinPath ? inputBinPath.value.trim() : '';
        ...
        if (window.pywebview?.api?.toggle_zapret) {
            ...
            if (res) {
                updateBadge(res.running, true);
                if (enable && !res.running) {
                    window.dispatchEvent(new CustomEvent('nedotify:toast', {
                        detail: { msg: 'Не удалось запустить winws.exe. Проверьте наличие бинарника', type: 'warning' }
                    }));
                } else if (enable) {
                    window.dispatchEvent(new CustomEvent('nedotify:toast', {
                        detail: { msg: 'Zapret (Обход DPI) успешно запущен!', type: 'success' }
                    }));
                }
            }
        }
    ...
    if (inputCustomArgs) {
        inputCustomArgs.addEventListener('change', () => {
            if (toggleZapret && toggleZapret.classList.contains('on')) {
                applyZapret(true);
            }
        });
    ...
    if (inputBinPath) {
        inputBinPath.addEventListener('change', () => {
            if (toggleZapret && toggleZapret.classList.contains('on')) {
                applyZapret(true);
            }
        });
        ...
        window.pywebview.api.get_zapret_status().then(status => {
            if (status) {
                ...
                if (status.mode) {
                    ...
                    document.querySelectorAll('#zapret-mode-select .opt-card').forEach(c => {
                        c.classList.toggle('active', c.dataset.val === status.mode);
                    });
                }
                if (inputCustomArgs && status.custom_args) inputCustomArgs.value = status.custom_args;
                if (inputBinPath && status.binary_path) inputBinPath.value = status.binary_path;
                updateBadge(status.running, status.binary_found);
            }
        }).catch(() => {});
    }
}
...
function setupPlayerSettingsPanel() {
    ...
    const mpPosGroup = document.getElementById('mp-pos-btn-group');
    if (mpPosGroup) {

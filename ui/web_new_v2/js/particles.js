...
export function initParticles() {
    ...
    function animate(timestamp) {
        ...
        if (activePage && activePage.id !== 'view-home') {
            animFrameId = null;
            return;
        }
...
window.addEventListener('nedotify:page_changed', (e) => {
    const pageId = e.detail;
    const isHomePage = (pageId === 'home');
    if (!isHomePage) {
        // Stop particles when leaving Home page
        if (animFrameId) {
            cancelAnimationFrame(animFrameId);
            animFrameId = null;
        }
    } else {
        // Resume particles on Home page if enabled
        const currSettings = window.settings || {};
        if (isParticlesRunning && !document.hidden && currSettings.ui?.particles_enabled !== false) {
            if (!animFrameId && animateFn) {
                lastFrameTime = performance.now();
